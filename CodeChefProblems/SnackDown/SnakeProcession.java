...
When he sees a snake first, it'll be its Head, and hence he will mark a 'H'. The snakes are long, and when he sees 
the snake finally slither away, he'll mark a 'T' to denote its tail. In the time in between, when the snake is 
...
"..H..T...HTH....T.", or "...", or "HT", whereas "T...H..H.T", "H..T..H", "H..H..T..T" would be invalid reports 
...
Chef had binged on the festival food and had been very drowsy. So his report might be invalid. You need to 
help him find out if his report is valid or not.

Input
	The first line contains a single integer, R, which denotes the number of reports to be checked. 
	The description of each report follows after this.

	The first line of each report contains a single integer, L, the length of that report.

	The second line of each report contains a string of length L. The string contains only the 
	characters '.', 'H', and 'T'.
Output
...
class SnakeProcession {
	public static void main(String args[]) {
		...
		int noOfReports = stdin.nextInt();
		String output[] = new String[noOfReports];
 
		for(int reports = 0; reports < noOfReports; reports++) {
			int length = stdin.nextInt();
			String snakesProStr = stdin.next();
			String repalcedHTString = snakesProStr.replaceAll("\\.", "").replaceAll("HT", "");
 
			if(repalcedHTString.length() > 0) {
				output[reports] = "Invalid";
			} else {
				output[reports] = "Valid";
			}
		}
